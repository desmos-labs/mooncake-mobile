...
const useFollowOrUnfollow = () => {
  ...
  const { handleOptimisticRelationship, resolveOptimisticRelationshipForAddress } =
  ...
  const followOrUnfollowUser = useRecoilCallback(
    ({ snapshot }) =>
      async ({ addrToFollow, stayOnCurrentScreen = true }: FollowOrUnfollowUserArgs) => {
        if (!activeAddress) throw new Error('No active address found');

        setLoading(true);

        const isAlreadyFollowing = await snapshot.getPromise(isFollowingAddr(addrToFollow));
        ...
        try {
          let msg: MsgDeleteRelationshipEncodeObject | MsgCreateRelationshipEncodeObject;
