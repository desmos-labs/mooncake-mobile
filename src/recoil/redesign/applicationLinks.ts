 ...
 */
export const useStoredApplicationLinks = () =>
  useRecoilValue(applicationLinksState);
...
const mergeApplicationLinks = (
  ...
  first
    ...
    .forEach(value => applicationLinksSet.add(value));
  second
    .map(link => JSON.stringify(link))
    .forEach(value => applicationLinksSet.add(value));
  return Array.from(applicationLinksSet).map(
    value => JSON.parse(value) as ApplicationLink,
  );
};
  ...
  return React.useCallback(
    (user: string, applicationLinks: ApplicationLink[]) => {
      setApplicationsLinks(currentApplicationLinks => {
        ...
        const mergedLinks = mergeApplicationLinks(
          exitingApplicationLinks,
          applicationLinks,
        );
