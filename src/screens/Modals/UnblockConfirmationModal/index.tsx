...
export type UnlockConfirmationModalParams = {
  /**
  ...
  onPressNo: () => void;
  /**
   * The name of the user to be unblocked
   */
  userToUnblock: string;
};

type NavProps = StackScreenProps<RootNavigatorParamList, ROUTES.UNBLOCK_CONFIRMATION_MODAL>;

/**
 * Modal to request authorization for simplified tx broadcasting
 * @constructor
 */
const UnblockConfirmationModal = () => {
  const {
    params: { onDismiss, onPressYes, onPressNo, userToUnblock },
    ...
    <View style={styles.container}>
      ...
      <View style={styles.innerContainer}>
        <Spacer paddingBottom={16}>
          <Typography.H6 style={CommonStyles.textAlign.center}>{t('unblock')}</Typography.H6>
        </Spacer>

        <Spacer paddingTop={theme.spacing.m} />
        <Typography.Body5 style={styles.body}>
          {t('content', { username: userToUnblock })}
        </Typography.Body5>
