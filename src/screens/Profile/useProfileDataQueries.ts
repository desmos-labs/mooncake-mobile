...
const useProfileDataQueries = () => {
  ...
  const { refetch: refetchProfileData, loading } = useGetProfileData(activeAddress!);
  ...
  const { address, bio, dtag, cover_pic, profile_pic, nickname } = profileData as ProfileData;
