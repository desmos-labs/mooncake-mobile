...
const useHooks = () => {
  const [notificationsWithProfile, setNotificationsWithProfile] = useState<any[]>([]);
  ...
  const refetch = useCallback(async () => {
    setRefetching(true);
    await notificationsRefetch().finally(() => setTimeout(() => setRefetching(false), 500));
  ...
  const notificationsData: null | any[] = useMemo(() => {
    ...
    sortedArray.forEach(
      (singleNotification: { timestamp: any; data: any; user_address: string }) => {
    ...
    } else if (thisWeekNotifications.length > 0 && earlierNotifications.length <= 0) {
